    <div class="py-10 px-5 bg-gray-200 transition duration-150 ease-in-out z-10 " >
        <div  class="container mx-auto w-11/12 max-w-lg">
            <div class="relative py-10 px-12 md:px-10 bg-white shadow-md rounded border border-gray-400">
              ...
              <h1 class="text-gray-800 font-lg font-bold tracking-normal leading-tight mb-4">Enter Shift Of Workings </h1>
                <x-guest-layout>
                    <x-slot name="logo">
                        ...
                        <div class="mb-6">
                            <x-jet-label class="text-gray-800 text-sm font-bold leading-tight tracking-normal" for="name" value="{{ __('Name-Shift') }}"/>
                            <x-jet-input id="name" class="block mt-1 w-full" type="text" wire:model.debounce.80ms="name"/>
                            @error('name') <span class="error">{{$message}}</span>@enderror
                        </div>

                        <div class="mt-4">
                            <x-jet-label for="duration" class="text-gray-800 text-sm font-bold leading-tight tracking-normal"    value="{{ __           ('Duration-time') }}"/>
                            <x-jet-input id="duration" class="block mt-1 w-full" type="text" wire:model.debounce.80ms="duration"/>
                            @error('duration time') <span class="error">{{$message}}</span>@enderror
                        </div>
